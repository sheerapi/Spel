...
static SpvReflectResult IntersectSortedAccessedVariable(
	const SpvReflectPrvAccessedVariable* p_arr0, size_t arr0_size, const uint32_t* p_arr1,
...
static SpvReflectResult ReadStr(SpvReflectPrvParser* p_parser, uint32_t word_offset,
								uint32_t word_index, uint32_t word_count,
											...
											SpvReflectPrvAccessChain* p_access_chain)
{
	...
	while (base_node->op != SpvOpVariable)
	{
		...
		break;
		case SpvOpBitcast:
			// This can be caused by something like GL_EXT_buffer_reference_uvec2 trying
			// to load a pointer. We currently call from a push constant, so no way to
			// have a reference loop back into the PC block
			return 0;
		default:
		{
			assert(false);
...
static SpvReflectResult ParseNodes(SpvReflectPrvParser* p_parser)
{
	...
	while (spirv_word_index < p_parser->spirv_word_count)
	{
		...
		case SpvOpLoad:
		{
			// Only load enough so OpDecorate can reference the node, skip the remaining
			// operands.
			CHECKED_READU32(p_parser, p_node->word_offset + 1, p_node->result_type_id);
			CHECKED_READU32(p_parser, p_node->word_offset + 2, p_node->result_id);
		}
...
// Returns true if user_type matches pattern or if user_type begins with pattern and the
// next character is ':' For example, UserTypeMatches("rwbuffer", "rwbuffer") will be
// true, UserTypeMatches("rwbuffer", "rwbuffer:<S>") will be true, and
												...
												SpvReflectShaderModule* p_module)
{
	...
	for (uint32_t descriptor_index = 0;
		 descriptor_index < p_module->descriptor_binding_count; ++descriptor_index)
	{
				...
				(p_member_type->type_flags & SPV_REFLECT_TYPE_FLAG_REF))
			{
				...
				if (!found_recursion)
				{
					...
					if (struct_type->struct_type_description)
					{
						...
						p_parser
							->physical_pointer_check[p_parser->physical_pointer_count] =
							p_member_type;
						p_parser->physical_pointer_count++;
						if (p_parser->physical_pointer_count >=
							MAX_RECURSIVE_PHYSICAL_POINTER_CHECK)
...
static SpvReflectResult ParseDescriptorBlockVariableSizes(
	SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module, bool is_parent_root,
	bool is_parent_aos, bool is_parent_rta, SpvReflectBlockVariable* p_var)
{
	...
	// Structs can offset order don't need to match the index order, so first order by
	// offset example:
	...
	// Parse padded size using offset difference for all member except for the last
	// entry...
	...
	// ...last entry just gets rounded up to near multiple of SPIRV_DATA_ALIGNMENT, which
	// is 16 and subtract the offset. last entry == entry with largest offset value
											  ...
											  SpvReflectShaderModule* p_module)
{
		 ...
		 descriptor_index < p_module->descriptor_binding_count; ++descriptor_index)
	{
		...
		SpvReflectResult result = ParseDescriptorBlockVariable(p_parser, p_module, p_type,
															   &p_descriptor->block);
		if (result != SPV_REFLECT_RESULT_SUCCESS)
		{
			return result;
		...
		for (uint32_t access_chain_index = 0;
			 access_chain_index < p_parser->access_chain_count; ++access_chain_index)
		{
			SpvReflectPrvAccessChain* p_access_chain =
				&(p_parser->access_chains[access_chain_index]);
			...
			result = ParseDescriptorBlockVariableUsage(p_parser, p_module, p_access_chain,
													   0, (SpvOp)INVALID_VALUE,
													   &p_descriptor->block);
			if (result != SPV_REFLECT_RESULT_SUCCESS)
			{
				return result;
		...
		result = ParseDescriptorBlockVariableSizes(p_parser, p_module, true, false,
												   is_parent_rta, &p_descriptor->block);
		if (result != SPV_REFLECT_RESULT_SUCCESS)
		{
			return result;
		}
									...
									SpvReflectFormat* p_format)
{
			 ...
			 (SPV_REFLECT_TYPE_FLAG_INT | SPV_REFLECT_TYPE_FLAG_BOOL))
	{
		...
		case 16:
			*p_format =
				signedness ? SPV_REFLECT_FORMAT_R16_SINT : SPV_REFLECT_FORMAT_R16_UINT;
			break;
			break;
		case 32:
			*p_format =
				signedness ? SPV_REFLECT_FORMAT_R32_SINT : SPV_REFLECT_FORMAT_R32_UINT;
			break;
			break;
		case 64:
			*p_format =
				signedness ? SPV_REFLECT_FORMAT_R64_SINT : SPV_REFLECT_FORMAT_R64_UINT;
			break;
		}
		result = SPV_REFLECT_RESULT_SUCCESS;
	}
...
static SpvReflectResult ParseInterfaceVariable(
	SpvReflectPrvParser* p_parser, const SpvReflectPrvDecorations* p_var_node_decorations,
												...
												uint32_t* p_interface_variable_ids)
{
	...
	for (size_t i = 0; i < interface_variable_count; ++i)
	{
		...
		SpvReflectResult result = ParseInterfaceVariable(
			p_parser, &p_node->decorations, &p_type_node->decorations, p_module, p_type,
			p_var, &has_built_in);
		if (result != SPV_REFLECT_RESULT_SUCCESS)
		{
			SPV_REFLECT_ASSERT(false);
...
static SpvReflectResult TraverseCallGraph(SpvReflectPrvParser* p_parser,
										  SpvReflectPrvFunction* p_func,
										  size_t* p_func_count, uint32_t* p_func_ids,
										  uint32_t depth)
{
	...
	{
		SpvReflectResult result = TraverseCallGraph(p_parser, p_func->callee_ptrs[i],
											   ...
											   SpvReflectDescriptorBinding* p_binding)
{
	...
	else if (p_next_node->op == SpvOpConstant)
	{
		...
		p_binding
			->byte_address_buffer_offsets[p_binding->byte_address_buffer_offset_count] =
			offset;
		p_binding->byte_address_buffer_offset_count++;
		return SPV_REFLECT_RESULT_SUCCESS;
	}

	// there is usually 2 (sometimes 3) instrucitons that make up the arithmetic logic to
	...
	for (uint32_t i = 0; i < count; i++)
	{
		...
		// All arithmetic ops takes 2 operands, assumption is the 2nd operand has the
		// constant
	...
	size_t push_constant_count, uint32_t* push_constants)
{
	...
	// If there are no used accessed, this is something like an empty function/early
	// return Basically there is going to be nothing to reflect, but everything after this
	// expects |p_used_accesses| to be allocated with real memory, see
	// https://github.com/KhronosGroup/SPIRV-Reflect/issues/319
		...
		if (byte_address_buffer_offset_count > 0)
		{
			...
			if (multi_entrypoint)
			{
				qsort(p_binding->byte_address_buffer_offsets,
					  ...
					  SortCompareUint32);
				p_binding->byte_address_buffer_offset_count = (uint32_t)DedupSortedUint32(
					p_binding->byte_address_buffer_offsets,
					p_binding->byte_address_buffer_offset_count);
			}
		}
										 ...
										 SpvReflectShaderModule* p_module)
{
	...
	for (size_t i = 0;
		 entry_point_index < p_parser->entry_point_count && i < p_parser->node_count; ++i)
	{
		SpvReflectPrvNode* p_node = &(p_parser->nodes[i]);
		...
		result =
			ParseStaticallyUsedResources(p_parser, p_module, p_entry_point, uniform_count,
...
SpvReflectResult spvReflectChangeDescriptorBindingNumbers(
	SpvReflectShaderModule* p_module, const SpvReflectDescriptorBinding* p_binding,
