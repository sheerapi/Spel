...
// Manual helpers
// These implement functionality that isn't in the original C++ API, but is useful to
// callers from other languages

CIMGUI_API void cimgui::ImVector_Construct(void* vector)
{
	// All ImVector classes are the same size, so it doesn't matter which we use for
	// sizeof() here
...
{
	// As with ImVector_construct(), it doesn't matter what the type parameter is here as
	// we just want to get the pointer and free it (without calling destructors or
	// anything similar)
...
// Because some of the functions involved are relatively new, we can't assume they exist
// and thus we make use of some #defines set during the generation process to decide if we
// want to include the code or not

#if defined(IMGUI_HAS_DOCK)

namespace
{
	// Data we use in the thunk to convert these from C++-style callbacks to C-style
	// callbacks
	struct ImGui_DearBindingsThunkData
	{
		void (*PlatformIO_GetWindowPos_ThunkTarget)(cimgui::ImGuiViewport* vp,
													cimgui::ImVec2* result);
		void (*PlatformIO_GetWindowSize_ThunkTarget)(cimgui::ImGuiViewport* vp,
													 cimgui::ImVec2* result);
#	ifdef IMGUI_DEAR_BINDINGS_HAS_GETWINDOWFRAMEBUFFERSCALE
		void (*PlatformIO_GetWindowFramebufferScale_ThunkTarget)(
			cimgui::ImGuiViewport* vp, cimgui::ImVec2* result);
#	endif
#	ifdef IMGUI_DEAR_BINDINGS_HAS_GETWINDOWWORKAREAINSETS
		void (*PlatformIO_GetWindowWorkAreaInsets_ThunkTarget)(cimgui::ImGuiViewport* vp,
...
#	endif
	...
	// Tidy up our thunk data, deleting it if all the target pointers are null (i.e. it is
	// unused)
	void ImGui_TidyDearBindingsThunkData()
	{
		...
		if (io.BackendLanguageUserData)
		{
			ImGui_DearBindingsThunkData* thunkData =
				reinterpret_cast<ImGui_DearBindingsThunkData*>(
...
#	endif
	...
	// Copies of the conversion stubs in order to deal with the fact that they are
	// declared later in the file Fixme: This is a little messy, but reordering things is
	// also fiddly to do
	...
	ImVec2 ImGuiPlatformIO_GetWindowPos_Thunk(ImGuiViewport* vp)
	{
		...
		thunkData->PlatformIO_GetWindowPos_ThunkTarget(
			reinterpret_cast<cimgui::ImGuiViewport*>(vp), &result);
...
CIMGUI_API void cimgui::ImGuiPlatformIO_SetPlatform_GetWindowPos(
	void (*func)(cimgui::ImGuiViewport* vp, cimgui::ImVec2* result))
{
	ImGui_DearBindingsThunkData* thunkData = ImGui_GetDearBindingsThunkData();
	thunkData->PlatformIO_GetWindowPos_ThunkTarget = func;
	::ImGui::GetPlatformIO().Platform_GetWindowPos =
		(func != nullptr) ? ImGuiPlatformIO_GetWindowPos_Thunk : nullptr;
	if (!func)
	{
		ImGui_TidyDearBindingsThunkData(); // Try to release thunk data if no longer
										   // required
	}
}

CIMGUI_API void cimgui::ImGuiPlatformIO_SetPlatform_GetWindowSize(
	void (*func)(cimgui::ImGuiViewport* vp, cimgui::ImVec2* result))
{
	ImGui_DearBindingsThunkData* thunkData = ImGui_GetDearBindingsThunkData();
	thunkData->PlatformIO_GetWindowSize_ThunkTarget = func;
	::ImGui::GetPlatformIO().Platform_GetWindowSize =
		(func != nullptr) ? ImGuiPlatformIO_GetWindowSize_Thunk : nullptr;
	if (!func)
	{
		ImGui_TidyDearBindingsThunkData(); // Try to release thunk data if no longer
										   // required
...
#	ifdef IMGUI_DEAR_BINDINGS_HAS_GETWINDOWFRAMEBUFFERSCALE
CIMGUI_API void cimgui::ImGuiPlatformIO_SetPlatform_GetWindowFramebufferScale(
	void (*func)(cimgui::ImGuiViewport* vp, cimgui::ImVec2* result))
{
	ImGui_DearBindingsThunkData* thunkData = ImGui_GetDearBindingsThunkData();
	thunkData->PlatformIO_GetWindowFramebufferScale_ThunkTarget = func;
	::ImGui::GetPlatformIO().Platform_GetWindowFramebufferScale =
		(func != nullptr) ? ImGuiPlatformIO_GetWindowFramebufferScale_Thunk : nullptr;
	if (!func)
	{
		ImGui_TidyDearBindingsThunkData(); // Try to release thunk data if no longer
										   // required
...
#	ifdef IMGUI_DEAR_BINDINGS_HAS_GETWINDOWWORKAREAINSETS
CIMGUI_API void cimgui::ImGuiPlatformIO_SetPlatform_GetWindowWorkAreaInsets(
	void (*func)(cimgui::ImGuiViewport* vp, cimgui::ImVec4* result))
{
	ImGui_DearBindingsThunkData* thunkData = ImGui_GetDearBindingsThunkData();
	thunkData->PlatformIO_GetWindowWorkAreaInsets_ThunkTarget = func;
	::ImGui::GetPlatformIO().Platform_GetWindowWorkAreaInsets =
		(func != nullptr) ? ImGuiPlatformIO_GetWindowWorkAreaInsets_Thunk : nullptr;
	if (!func)
	{
		ImGui_TidyDearBindingsThunkData(); // Try to release thunk data if no longer
										   // required
...
CIMGUI_API cimgui::ImGuiContext* cimgui::ImGui_CreateContext(
	cimgui::ImFontAtlas* shared_font_atlas)
{
	return reinterpret_cast<::cimgui::ImGuiContext*>(
		::ImGui::CreateContext(reinterpret_cast<::ImFontAtlas*>(shared_font_atlas)));
}
...
CIMGUI_API bool cimgui::ImGui_BeginChild(const char* str_id, cimgui::ImVec2 size,
...
CIMGUI_API void cimgui::ImGui_SetNextWindowPosEx(cimgui::ImVec2 pos, ImGuiCond cond,
												 cimgui::ImVec2 pivot)
{
	...
	cimgui::ImGuiSizeCallback custom_callback, void* custom_callback_data)
{
	::ImGui::SetNextWindowSizeConstraints(
		...
		reinterpret_cast<::ImGuiSizeCallback>(custom_callback), custom_callback_data);
}
...
CIMGUI_API void cimgui::ImGui_SetWindowPosStr(const char* name, cimgui::ImVec2 pos,
											  ImGuiCond cond)
{
	::ImGui::SetWindowPos(name, ConvertToCPP_ImVec2(pos), cond);
}

CIMGUI_API void cimgui::ImGui_SetWindowSizeStr(const char* name, cimgui::ImVec2 size,
											   ImGuiCond cond)
{
	::ImGui::SetWindowSize(name, ConvertToCPP_ImVec2(size), cond);
}

CIMGUI_API void cimgui::ImGui_SetWindowCollapsedStr(const char* name, bool collapsed,
													ImGuiCond cond)
{
...
CIMGUI_API void cimgui::ImGui_PushFontFloat(cimgui::ImFont* font,
...
CIMGUI_API bool cimgui::ImGui_InvisibleButton(const char* str_id, cimgui::ImVec2 size,
...
CIMGUI_API bool cimgui::ImGui_CheckboxFlagsUintPtr(const char* label, unsigned int* flags,
...
CIMGUI_API void cimgui::ImGui_ProgressBar(float fraction, cimgui::ImVec2 size_arg,
										  const char* overlay)
{
...
CIMGUI_API void cimgui::ImGui_Image(cimgui::ImTextureRef tex_ref,
									cimgui::ImVec2 image_size)
{
	::ImGui::Image(ConvertToCPP_ImTextureRef(tex_ref), ConvertToCPP_ImVec2(image_size));
}

CIMGUI_API void cimgui::ImGui_ImageEx(cimgui::ImTextureRef tex_ref,
...
CIMGUI_API void cimgui::ImGui_ImageWithBg(cimgui::ImTextureRef tex_ref,
										  cimgui::ImVec2 image_size)
{
	::ImGui::ImageWithBg(ConvertToCPP_ImTextureRef(tex_ref),
						 ConvertToCPP_ImVec2(image_size));
}

CIMGUI_API void cimgui::ImGui_ImageWithBgEx(cimgui::ImTextureRef tex_ref,
											cimgui::ImVec2 image_size, cimgui::ImVec2 uv0,
...
CIMGUI_API bool cimgui::ImGui_ImageButton(const char* str_id,
										  cimgui::ImTextureRef tex_ref,
										  cimgui::ImVec2 image_size)
{
	return ::ImGui::ImageButton(str_id, ConvertToCPP_ImTextureRef(tex_ref),
								ConvertToCPP_ImVec2(image_size));
}

CIMGUI_API bool cimgui::ImGui_ImageButtonEx(const char* str_id,
											cimgui::ImTextureRef tex_ref,
											cimgui::ImVec2 image_size, cimgui::ImVec2 uv0,
											cimgui::ImVec2 uv1, cimgui::ImVec4 bg_col,
											cimgui::ImVec4 tint_col)
...
CIMGUI_API bool cimgui::ImGui_BeginCombo(const char* label, const char* preview_value,
...
CIMGUI_API bool cimgui::ImGui_ComboCallback(const char* label, int* current_item,
...
CIMGUI_API bool cimgui::ImGui_ComboCallbackEx(
	const char* label, int* current_item, const char* (*getter)(void* user_data, int idx),
...
CIMGUI_API bool cimgui::ImGui_DragFloatEx(const char* label, float* v, float v_speed,
...
CIMGUI_API bool cimgui::ImGui_DragFloat2Ex(const char* label, float v[2], float v_speed,
										   float v_min, float v_max, const char* format,
										   ImGuiSliderFlags flags)
{
...
CIMGUI_API bool cimgui::ImGui_DragFloat3Ex(const char* label, float v[3], float v_speed,
										   float v_min, float v_max, const char* format,
										   ImGuiSliderFlags flags)
{
...
CIMGUI_API bool cimgui::ImGui_DragFloat4Ex(const char* label, float v[4], float v_speed,
										   float v_min, float v_max, const char* format,
										   ImGuiSliderFlags flags)
{
...
CIMGUI_API bool cimgui::ImGui_DragFloatRange2(const char* label, float* v_current_min,
											  float* v_current_max)
{
	return ::ImGui::DragFloatRange2(label, v_current_min, v_current_max);
}

CIMGUI_API bool cimgui::ImGui_DragFloatRange2Ex(const char* label, float* v_current_min,
...
CIMGUI_API bool cimgui::ImGui_DragIntEx(const char* label, int* v, float v_speed,
...
CIMGUI_API bool cimgui::ImGui_DragInt2Ex(const char* label, int v[2], float v_speed,
										 int v_min, int v_max, const char* format,
										 ImGuiSliderFlags flags)
{
...
CIMGUI_API bool cimgui::ImGui_DragInt3Ex(const char* label, int v[3], float v_speed,
										 int v_min, int v_max, const char* format,
										 ImGuiSliderFlags flags)
{
...
CIMGUI_API bool cimgui::ImGui_DragInt4Ex(const char* label, int v[4], float v_speed,
										 int v_min, int v_max, const char* format,
										 ImGuiSliderFlags flags)
{
...
CIMGUI_API bool cimgui::ImGui_DragIntRange2(const char* label, int* v_current_min,
											int* v_current_max)
{
	return ::ImGui::DragIntRange2(label, v_current_min, v_current_max);
}

CIMGUI_API bool cimgui::ImGui_DragIntRange2Ex(const char* label, int* v_current_min,
											  int* v_current_max, float v_speed,
											  int v_min, int v_max, const char* format,
											  const char* format_max,
											  ImGuiSliderFlags flags)
{
	return ::ImGui::DragIntRange2(label, v_current_min, v_current_max, v_speed, v_min,
								  v_max, format, format_max, flags);
}

CIMGUI_API bool cimgui::ImGui_DragScalar(const char* label, ImGuiDataType data_type,
										 void* p_data)
{
	return ::ImGui::DragScalar(label, data_type, p_data);
}

CIMGUI_API bool cimgui::ImGui_DragScalarEx(const char* label, ImGuiDataType data_type,
...
CIMGUI_API bool cimgui::ImGui_DragScalarN(const char* label, ImGuiDataType data_type,
										  void* p_data, int components)
{
	return ::ImGui::DragScalarN(label, data_type, p_data, components);
}

CIMGUI_API bool cimgui::ImGui_DragScalarNEx(const char* label, ImGuiDataType data_type,
											void* p_data, int components, float v_speed,
											const void* p_min, const void* p_max,
											const char* format, ImGuiSliderFlags flags)
{
	return ::ImGui::DragScalarN(label, data_type, p_data, components, v_speed, p_min,
								p_max, format, flags);
}

CIMGUI_API bool cimgui::ImGui_SliderFloat(const char* label, float* v, float v_min,
										  float v_max)
{
	return ::ImGui::SliderFloat(label, v, v_min, v_max);
}

CIMGUI_API bool cimgui::ImGui_SliderFloatEx(const char* label, float* v, float v_min,
...
CIMGUI_API bool cimgui::ImGui_SliderFloat2(const char* label, float v[2], float v_min,
										   float v_max)
{
	return ::ImGui::SliderFloat2(label, v, v_min, v_max);
}

CIMGUI_API bool cimgui::ImGui_SliderFloat2Ex(const char* label, float v[2], float v_min,
											 float v_max, const char* format,
											 ImGuiSliderFlags flags)
{
...
CIMGUI_API bool cimgui::ImGui_SliderFloat3(const char* label, float v[3], float v_min,
										   float v_max)
{
	return ::ImGui::SliderFloat3(label, v, v_min, v_max);
}

CIMGUI_API bool cimgui::ImGui_SliderFloat3Ex(const char* label, float v[3], float v_min,
											 float v_max, const char* format,
											 ImGuiSliderFlags flags)
{
...
CIMGUI_API bool cimgui::ImGui_SliderFloat4(const char* label, float v[4], float v_min,
										   float v_max)
{
	return ::ImGui::SliderFloat4(label, v, v_min, v_max);
}

CIMGUI_API bool cimgui::ImGui_SliderFloat4Ex(const char* label, float v[4], float v_min,
											 float v_max, const char* format,
											 ImGuiSliderFlags flags)
{
...
CIMGUI_API bool cimgui::ImGui_SliderIntEx(const char* label, int* v, int v_min, int v_max,
										  const char* format, ImGuiSliderFlags flags)
{
...
CIMGUI_API bool cimgui::ImGui_SliderInt2Ex(const char* label, int v[2], int v_min,
...
CIMGUI_API bool cimgui::ImGui_SliderInt3Ex(const char* label, int v[3], int v_min,
										   int v_max, const char* format,
										   ImGuiSliderFlags flags)
{
...
CIMGUI_API bool cimgui::ImGui_SliderInt4Ex(const char* label, int v[4], int v_min,
										   int v_max, const char* format,
										   ImGuiSliderFlags flags)
{
...
CIMGUI_API bool cimgui::ImGui_SliderScalar(const char* label, ImGuiDataType data_type,
...
CIMGUI_API bool cimgui::ImGui_SliderScalarEx(const char* label, ImGuiDataType data_type,
											 void* p_data, const void* p_min,
											 const void* p_max, const char* format,
											 ImGuiSliderFlags flags)
{
...
CIMGUI_API bool cimgui::ImGui_SliderScalarN(const char* label, ImGuiDataType data_type,
											void* p_data, int components,
											const void* p_min, const void* p_max)
{
...
CIMGUI_API bool cimgui::ImGui_SliderScalarNEx(const char* label, ImGuiDataType data_type,
											  void* p_data, int components,
											  const void* p_min, const void* p_max,
											  const char* format, ImGuiSliderFlags flags)
{
...
CIMGUI_API bool cimgui::ImGui_VSliderFloat(const char* label, cimgui::ImVec2 size,
										   float* v, float v_min, float v_max)
{
	return ::ImGui::VSliderFloat(label, ConvertToCPP_ImVec2(size), v, v_min, v_max);
}

CIMGUI_API bool cimgui::ImGui_VSliderFloatEx(const char* label, cimgui::ImVec2 size,
											 float* v, float v_min, float v_max,
											 const char* format, ImGuiSliderFlags flags)
{
	return ::ImGui::VSliderFloat(label, ConvertToCPP_ImVec2(size), v, v_min, v_max,
...
CIMGUI_API bool cimgui::ImGui_VSliderInt(const char* label, cimgui::ImVec2 size, int* v,
										 int v_min, int v_max)
{
	return ::ImGui::VSliderInt(label, ConvertToCPP_ImVec2(size), v, v_min, v_max);
}

CIMGUI_API bool cimgui::ImGui_VSliderIntEx(const char* label, cimgui::ImVec2 size, int* v,
										   int v_min, int v_max, const char* format,
										   ImGuiSliderFlags flags)
{
	return ::ImGui::VSliderInt(label, ConvertToCPP_ImVec2(size), v, v_min, v_max, format,
							   flags);
}

CIMGUI_API bool cimgui::ImGui_VSliderScalar(const char* label, cimgui::ImVec2 size,
											ImGuiDataType data_type, void* p_data,
											const void* p_min, const void* p_max)
{
	return ::ImGui::VSliderScalar(label, ConvertToCPP_ImVec2(size), data_type, p_data,
								  p_min, p_max);
}

CIMGUI_API bool cimgui::ImGui_VSliderScalarEx(const char* label, cimgui::ImVec2 size,
											  ImGuiDataType data_type, void* p_data,
											  const void* p_min, const void* p_max,
											  const char* format, ImGuiSliderFlags flags)
{
...
CIMGUI_API bool cimgui::ImGui_InputText(const char* label, char* buf, size_t buf_size,
										ImGuiInputTextFlags flags)
{
	return ::ImGui::InputText(label, buf, buf_size, flags);
}

CIMGUI_API bool cimgui::ImGui_InputTextEx(const char* label, char* buf, size_t buf_size,
										  ImGuiInputTextFlags flags,
										  cimgui::ImGuiInputTextCallback callback,
										  void* user_data)
{
	return ::ImGui::InputText(label, buf, buf_size, flags,
							  reinterpret_cast<::ImGuiInputTextCallback>(callback),
							  user_data);
}
...
CIMGUI_API bool cimgui::ImGui_InputTextWithHint(const char* label, const char* hint,
...
CIMGUI_API bool cimgui::ImGui_InputTextWithHintEx(const char* label, const char* hint,
												  char* buf, size_t buf_size,
												  ImGuiInputTextFlags flags,
												  cimgui::ImGuiInputTextCallback callback,
												  void* user_data)
...
CIMGUI_API bool cimgui::ImGui_InputFloatEx(const char* label, float* v, float step,
...
CIMGUI_API bool cimgui::ImGui_InputDoubleEx(const char* label, double* v, double step,
...
CIMGUI_API bool cimgui::ImGui_InputScalar(const char* label, ImGuiDataType data_type,
										  void* p_data)
{
	return ::ImGui::InputScalar(label, data_type, p_data);
}

CIMGUI_API bool cimgui::ImGui_InputScalarEx(const char* label, ImGuiDataType data_type,
											void* p_data, const void* p_step,
											const void* p_step_fast, const char* format,
											ImGuiInputTextFlags flags)
{
	return ::ImGui::InputScalar(label, data_type, p_data, p_step, p_step_fast, format,
								flags);
}

CIMGUI_API bool cimgui::ImGui_InputScalarN(const char* label, ImGuiDataType data_type,
										   void* p_data, int components)
{
	return ::ImGui::InputScalarN(label, data_type, p_data, components);
}

CIMGUI_API bool cimgui::ImGui_InputScalarNEx(const char* label, ImGuiDataType data_type,
											 void* p_data, int components,
...
CIMGUI_API bool cimgui::ImGui_ColorButton(const char* desc_id, cimgui::ImVec4 col,
										  ImGuiColorEditFlags flags)
{
	return ::ImGui::ColorButton(desc_id, ConvertToCPP_ImVec4(col), flags);
}

CIMGUI_API bool cimgui::ImGui_ColorButtonEx(const char* desc_id, cimgui::ImVec4 col,
											ImGuiColorEditFlags flags,
											cimgui::ImVec2 size)
{
	return ::ImGui::ColorButton(desc_id, ConvertToCPP_ImVec4(col), flags,
...
CIMGUI_API bool cimgui::ImGui_TreeNodeExStr(const char* str_id, ImGuiTreeNodeFlags flags,
											const char* fmt, ...)
{
	va_list args;
	va_start(args, fmt);
...
CIMGUI_API bool cimgui::ImGui_TreeNodeExPtr(const void* ptr_id, ImGuiTreeNodeFlags flags,
											const char* fmt, ...)
{
	va_list args;
	va_start(args, fmt);
...
CIMGUI_API bool cimgui::ImGui_TreeNodeExV(const char* str_id, ImGuiTreeNodeFlags flags,
										  const char* fmt, va_list args)
{
	return ::ImGui::TreeNodeExV(str_id, flags, fmt, args);
}

CIMGUI_API bool cimgui::ImGui_TreeNodeExVPtr(const void* ptr_id, ImGuiTreeNodeFlags flags,
											 const char* fmt, va_list args)
{
...
CIMGUI_API bool cimgui::ImGui_CollapsingHeaderBoolPtr(const char* label, bool* p_visible,
													  ImGuiTreeNodeFlags flags)
{
...
CIMGUI_API bool cimgui::ImGui_SelectableBoolPtr(const char* label, bool* p_selected,
												ImGuiSelectableFlags flags)
{
	return ::ImGui::Selectable(label, p_selected, flags);
}

CIMGUI_API bool cimgui::ImGui_SelectableBoolPtrEx(const char* label, bool* p_selected,
												  ImGuiSelectableFlags flags,
												  cimgui::ImVec2 size)
{
...
CIMGUI_API cimgui::ImGuiMultiSelectIO* cimgui::ImGui_BeginMultiSelect(
...
CIMGUI_API cimgui::ImGuiMultiSelectIO* cimgui::ImGui_BeginMultiSelectEx(
...
CIMGUI_API bool cimgui::ImGui_ListBoxCallback(const char* label, int* current_item,
											  const char* (*getter)(void* user_data,
																	int idx),
											  void* user_data, int items_count)
{
...
CIMGUI_API bool cimgui::ImGui_ListBoxCallbackEx(
	const char* label, int* current_item, const char* (*getter)(void* user_data, int idx),
...
CIMGUI_API void cimgui::ImGui_PlotLinesEx(const char* label, const float* values,
										  int values_count, int values_offset,
										  const char* overlay_text, float scale_min,
										  float scale_max, cimgui::ImVec2 graph_size,
										  int stride)
{
	::ImGui::PlotLines(label, values, values_count, values_offset, overlay_text,
					   scale_min, scale_max, ConvertToCPP_ImVec2(graph_size), stride);
}

CIMGUI_API void cimgui::ImGui_PlotLinesCallback(const char* label,
...
CIMGUI_API void cimgui::ImGui_PlotHistogram(const char* label, const float* values,
											int values_count)
{
	::ImGui::PlotHistogram(label, values, values_count);
}

CIMGUI_API void cimgui::ImGui_PlotHistogramEx(const char* label, const float* values,
											  int values_count, int values_offset,
											  const char* overlay_text, float scale_min,
											  float scale_max, cimgui::ImVec2 graph_size,
											  int stride)
...
CIMGUI_API void cimgui::ImGui_PlotHistogramCallback(const char* label,
													float (*values_getter)(void* data,
																		   int idx),
													void* data, int values_count)
{
...
CIMGUI_API bool cimgui::ImGui_MenuItemEx(const char* label, const char* shortcut,
										 bool selected, bool enabled)
{
	return ::ImGui::MenuItem(label, shortcut, selected, enabled);
}

CIMGUI_API bool cimgui::ImGui_MenuItemBoolPtr(const char* label, const char* shortcut,
...
CIMGUI_API bool cimgui::ImGui_BeginPopupContextItemEx(const char* str_id,
													  ImGuiPopupFlags popup_flags)
{
...
CIMGUI_API bool cimgui::ImGui_BeginPopupContextWindowEx(const char* str_id,
														ImGuiPopupFlags popup_flags)
{
...
CIMGUI_API bool cimgui::ImGui_BeginPopupContextVoidEx(const char* str_id,
													  ImGuiPopupFlags popup_flags)
{
...
CIMGUI_API bool cimgui::ImGui_BeginTableEx(const char* str_id, int columns,
										   ImGuiTableFlags flags,
										   cimgui::ImVec2 outer_size, float inner_width)
{
	return ::ImGui::BeginTable(str_id, columns, flags, ConvertToCPP_ImVec2(outer_size),
							   inner_width);
}
...
CIMGUI_API void cimgui::ImGui_TableSetBgColor(ImGuiTableBgTarget target, ImU32 color,
											  int column_n)
{
...
CIMGUI_API ImGuiID cimgui::ImGui_DockSpaceEx(ImGuiID dockspace_id, cimgui::ImVec2 size,
											 ImGuiDockNodeFlags flags,
											 const cimgui::ImGuiWindowClass* window_class)
{
	return ::ImGui::DockSpace(dockspace_id, ConvertToCPP_ImVec2(size), flags,
...
CIMGUI_API ImGuiID cimgui::ImGui_DockSpaceOverViewportEx(
	ImGuiID dockspace_id, const cimgui::ImGuiViewport* viewport, ImGuiDockNodeFlags flags,
	const cimgui::ImGuiWindowClass* window_class)
{
	return ::ImGui::DockSpaceOverViewport(
...
CIMGUI_API void cimgui::ImGui_SetNextWindowClass(
	const cimgui::ImGuiWindowClass* window_class)
{
...
CIMGUI_API bool cimgui::ImGui_SetDragDropPayload(const char* type, const void* data,
...
CIMGUI_API const cimgui::ImGuiPayload* cimgui::ImGui_AcceptDragDropPayload(
...
CIMGUI_API void cimgui::ImGui_PushClipRect(cimgui::ImVec2 clip_rect_min,
...
CIMGUI_API cimgui::ImDrawList* cimgui::ImGui_GetBackgroundDrawListEx(
	cimgui::ImGuiViewport* viewport)
{
	return reinterpret_cast<::cimgui::ImDrawList*>(
		::ImGui::GetBackgroundDrawList(reinterpret_cast<::ImGuiViewport*>(viewport)));
}
...
CIMGUI_API cimgui::ImDrawList* cimgui::ImGui_GetForegroundDrawListEx(
	cimgui::ImGuiViewport* viewport)
{
	return reinterpret_cast<::cimgui::ImDrawList*>(
		::ImGui::GetForegroundDrawList(reinterpret_cast<::ImGuiViewport*>(viewport)));
}
...
CIMGUI_API cimgui::ImVec2 cimgui::ImGui_CalcTextSizeEx(const char* text,
													   const char* text_end,
...
CIMGUI_API int cimgui::ImGui_GetKeyPressedAmount(cimgui::ImGuiKey key, float repeat_delay,
												 float rate)
{
...
CIMGUI_API cimgui::ImVec2 cimgui::ImGui_GetMouseDragDelta(ImGuiMouseButton button,
														  float lock_threshold)
{
...
{
	return ::ImGui::DebugCheckVersionAndDataLayout(version_str, sz_io, sz_style, sz_vec2,
...
CIMGUI_API void cimgui::ImGui_SetAllocatorFunctions(cimgui::ImGuiMemAllocFunc alloc_func,
													cimgui::ImGuiMemFreeFunc free_func,
													void* user_data)
{
	::ImGui::SetAllocatorFunctions(reinterpret_cast<::ImGuiMemAllocFunc>(alloc_func),
								   reinterpret_cast<::ImGuiMemFreeFunc>(free_func),
								   user_data);
}

CIMGUI_API void cimgui::ImGui_GetAllocatorFunctions(
	cimgui::ImGuiMemAllocFunc* p_alloc_func, cimgui::ImGuiMemFreeFunc* p_free_func,
	void** p_user_data)
{
	::ImGui::GetAllocatorFunctions(reinterpret_cast<::ImGuiMemAllocFunc*>(p_alloc_func),
								   reinterpret_cast<::ImGuiMemFreeFunc*>(p_free_func),
								   p_user_data);
}
...
CIMGUI_API void cimgui::ImGui_RenderPlatformWindowsDefaultEx(void* platform_render_arg,
...
CIMGUI_API cimgui::ImGuiViewport* cimgui::ImGui_FindViewportByID(ImGuiID viewport_id)
{
	return reinterpret_cast<::cimgui::ImGuiViewport*>(
		::ImGui::FindViewportByID(viewport_id));
}

CIMGUI_API cimgui::ImGuiViewport* cimgui::ImGui_FindViewportByPlatformHandle(
...
CIMGUI_API void cimgui::ImGuiIO_AddKeyEvent(cimgui::ImGuiIO* self, cimgui::ImGuiKey key,
											bool down)
{
...
CIMGUI_API void cimgui::ImGuiIO_AddKeyAnalogEvent(cimgui::ImGuiIO* self,
												  ...
												  float v)
{
	reinterpret_cast<::ImGuiIO*>(self)->AddKeyAnalogEvent(static_cast<::ImGuiKey>(key),
														  down, v);
}
...
CIMGUI_API void cimgui::ImGuiIO_AddMouseButtonEvent(cimgui::ImGuiIO* self, int button,
													bool down)
{
	reinterpret_cast<::ImGuiIO*>(self)->AddMouseButtonEvent(button, down);
}

CIMGUI_API void cimgui::ImGuiIO_AddMouseWheelEvent(cimgui::ImGuiIO* self, float wheel_x,
												   float wheel_y)
{
	reinterpret_cast<::ImGuiIO*>(self)->AddMouseWheelEvent(wheel_x, wheel_y);
}

CIMGUI_API void cimgui::ImGuiIO_AddMouseSourceEvent(cimgui::ImGuiIO* self,
...
CIMGUI_API void cimgui::ImGuiInputTextCallbackData_SetSelection(
	cimgui::ImGuiInputTextCallbackData* self, int s, int e)
{
...
CIMGUI_API bool cimgui::ImGuiPayload_IsDataType(const cimgui::ImGuiPayload* self,
												const char* type)
{
...
CIMGUI_API void cimgui::ImGuiTextFilter_ImGuiTextRange_split(
	...
	cimgui::ImVector_ImGuiTextRange* out)
{
	reinterpret_cast<const ::ImGuiTextFilter::ImGuiTextRange*>(self)->split(
		separator, reinterpret_cast<::ImVector<::ImGuiTextFilter::ImGuiTextRange>*>(out));
}
...
CIMGUI_API bool cimgui::ImGuiTextFilter_PassFilter(const cimgui::ImGuiTextFilter* self,
...
CIMGUI_API int cimgui::ImGuiStorage_GetInt(const cimgui::ImGuiStorage* self, ImGuiID key,
										   int default_val)
{
	return reinterpret_cast<const ::ImGuiStorage*>(self)->GetInt(key, default_val);
}

CIMGUI_API void cimgui::ImGuiStorage_SetInt(cimgui::ImGuiStorage* self, ImGuiID key,
											int val)
{
...
CIMGUI_API void cimgui::ImGuiStorage_SetBool(cimgui::ImGuiStorage* self, ImGuiID key,
											 bool val)
{
...
CIMGUI_API void cimgui::ImGuiStorage_SetFloat(cimgui::ImGuiStorage* self, ImGuiID key,
											  float val)
{
	reinterpret_cast<::ImGuiStorage*>(self)->SetFloat(key, val);
}

CIMGUI_API void* cimgui::ImGuiStorage_GetVoidPtr(const cimgui::ImGuiStorage* self,
												 ImGuiID key)
{
	return reinterpret_cast<const ::ImGuiStorage*>(self)->GetVoidPtr(key);
}

CIMGUI_API void cimgui::ImGuiStorage_SetVoidPtr(cimgui::ImGuiStorage* self, ImGuiID key,
												void* val)
{
	reinterpret_cast<::ImGuiStorage*>(self)->SetVoidPtr(key, val);
}

CIMGUI_API int* cimgui::ImGuiStorage_GetIntRef(cimgui::ImGuiStorage* self, ImGuiID key,
											   int default_val)
{
	return reinterpret_cast<::ImGuiStorage*>(self)->GetIntRef(key, default_val);
}

CIMGUI_API bool* cimgui::ImGuiStorage_GetBoolRef(cimgui::ImGuiStorage* self, ImGuiID key,
												 bool default_val)
{
...
CIMGUI_API void cimgui::ImGuiListClipper_IncludeItemByIndex(
	cimgui::ImGuiListClipper* self, int item_index)
{
	reinterpret_cast<::ImGuiListClipper*>(self)->IncludeItemByIndex(item_index);
}

CIMGUI_API void cimgui::ImGuiListClipper_IncludeItemsByIndex(
...
CIMGUI_API void cimgui::ImGuiListClipper_SeekCursorForItem(cimgui::ImGuiListClipper* self,
														   int item_index)
{
	reinterpret_cast<::ImGuiListClipper*>(self)->SeekCursorForItem(item_index);
}

CIMGUI_API void cimgui::ImColor_SetHSV(cimgui::ImColor* self, float h, float s, float v,
									   float a)
{
...
CIMGUI_API void cimgui::ImGuiSelectionBasicStorage_ApplyRequests(
	cimgui::ImGuiSelectionBasicStorage* self, cimgui::ImGuiMultiSelectIO* ms_io)
{
...
CIMGUI_API void cimgui::ImGuiSelectionBasicStorage_Swap(
	cimgui::ImGuiSelectionBasicStorage* self, cimgui::ImGuiSelectionBasicStorage* r)
{
...
CIMGUI_API bool cimgui::ImGuiSelectionBasicStorage_GetNextSelectedItem(
	cimgui::ImGuiSelectionBasicStorage* self, void** opaque_it, ImGuiID* out_id)
{
	return reinterpret_cast<::ImGuiSelectionBasicStorage*>(self)->GetNextSelectedItem(
		opaque_it, out_id);
}
...
{
	return reinterpret_cast<::ImGuiSelectionBasicStorage*>(self)->GetStorageIdFromIndex(
		idx);
}

CIMGUI_API void cimgui::ImGuiSelectionExternalStorage_ApplyRequests(
	cimgui::ImGuiSelectionExternalStorage* self, cimgui::ImGuiMultiSelectIO* ms_io)
{
...
CIMGUI_API void cimgui::ImDrawListSplitter_ClearFreeMemory(
	cimgui::ImDrawListSplitter* self)
{
	reinterpret_cast<::ImDrawListSplitter*>(self)->ClearFreeMemory();
}

CIMGUI_API void cimgui::ImDrawListSplitter_Split(cimgui::ImDrawListSplitter* self,
...
CIMGUI_API void cimgui::ImDrawListSplitter_Merge(cimgui::ImDrawListSplitter* self,
...
CIMGUI_API void cimgui::ImDrawListSplitter_SetCurrentChannel(
	cimgui::ImDrawListSplitter* self, cimgui::ImDrawList* draw_list, int channel_idx)
{
	reinterpret_cast<::ImDrawListSplitter*>(self)->SetCurrentChannel(
		reinterpret_cast<::ImDrawList*>(draw_list), channel_idx);
}

CIMGUI_API void cimgui::ImDrawList_PushClipRect(cimgui::ImDrawList* self,
												cimgui::ImVec2 clip_rect_min,
												cimgui::ImVec2 clip_rect_max,
												bool intersect_with_current_clip_rect)
{
...
CIMGUI_API void cimgui::ImDrawList_PushTexture(cimgui::ImDrawList* self,
											   cimgui::ImTextureRef tex_ref)
{
	reinterpret_cast<::ImDrawList*>(self)->PushTexture(
		ConvertToCPP_ImTextureRef(tex_ref));
}
...
CIMGUI_API cimgui::ImVec2 cimgui::ImDrawList_GetClipRectMin(
	const cimgui::ImDrawList* self)
{
	return ConvertFromCPP_ImVec2(
		reinterpret_cast<const ::ImDrawList*>(self)->GetClipRectMin());
}

CIMGUI_API cimgui::ImVec2 cimgui::ImDrawList_GetClipRectMax(
	const cimgui::ImDrawList* self)
{
	return ConvertFromCPP_ImVec2(
...
CIMGUI_API void cimgui::ImDrawList_AddLine(cimgui::ImDrawList* self, cimgui::ImVec2 p1,
...
CIMGUI_API void cimgui::ImDrawList_AddLineEx(cimgui::ImDrawList* self, cimgui::ImVec2 p1,
											 cimgui::ImVec2 p2, ImU32 col,
											 float thickness)
{
...
CIMGUI_API void cimgui::ImDrawList_AddRect(cimgui::ImDrawList* self, cimgui::ImVec2 p_min,
										   cimgui::ImVec2 p_max, ImU32 col)
{
	reinterpret_cast<::ImDrawList*>(self)->AddRect(ConvertToCPP_ImVec2(p_min),
												   ConvertToCPP_ImVec2(p_max), col);
}

CIMGUI_API void cimgui::ImDrawList_AddRectEx(cimgui::ImDrawList* self,
											 cimgui::ImVec2 p_min, cimgui::ImVec2 p_max,
...
CIMGUI_API void cimgui::ImDrawList_AddRectFilled(cimgui::ImDrawList* self,
												 ...
												 cimgui::ImVec2 p_max, ImU32 col)
{
	reinterpret_cast<::ImDrawList*>(self)->AddRectFilled(ConvertToCPP_ImVec2(p_min),
														 ConvertToCPP_ImVec2(p_max), col);
}
												   ...
												   float rounding, ImDrawFlags flags)
{
	reinterpret_cast<::ImDrawList*>(self)->AddRectFilled(
		ConvertToCPP_ImVec2(p_min), ConvertToCPP_ImVec2(p_max), col, rounding, flags);
}
...
CIMGUI_API void cimgui::ImDrawList_AddQuad(cimgui::ImDrawList* self, cimgui::ImVec2 p1,
...
CIMGUI_API void cimgui::ImDrawList_AddQuadEx(cimgui::ImDrawList* self, cimgui::ImVec2 p1,
											 cimgui::ImVec2 p2, cimgui::ImVec2 p3,
											 cimgui::ImVec2 p4, ImU32 col,
											 float thickness)
{
											   ...
											   cimgui::ImVec2 p3, ImU32 col)
{
	reinterpret_cast<::ImDrawList*>(self)->AddTriangle(
		ConvertToCPP_ImVec2(p1), ConvertToCPP_ImVec2(p2), ConvertToCPP_ImVec2(p3), col);
}
...
CIMGUI_API void cimgui::ImDrawList_AddCircle(cimgui::ImDrawList* self,
											 ...
											 ImU32 col)
{
	reinterpret_cast<::ImDrawList*>(self)->AddCircle(ConvertToCPP_ImVec2(center), radius,
													 col);
}
...
CIMGUI_API void cimgui::ImDrawList_AddCircleFilled(cimgui::ImDrawList* self,
												   ...
												   ImU32 col, int num_segments)
{
	reinterpret_cast<::ImDrawList*>(self)->AddCircleFilled(ConvertToCPP_ImVec2(center),
														   radius, col, num_segments);
}

CIMGUI_API void cimgui::ImDrawList_AddNgon(cimgui::ImDrawList* self,
										   cimgui::ImVec2 center, float radius, ImU32 col,
										   int num_segments)
{
	reinterpret_cast<::ImDrawList*>(self)->AddNgon(ConvertToCPP_ImVec2(center), radius,
												   col, num_segments);
}
...
CIMGUI_API void cimgui::ImDrawList_AddNgonFilled(cimgui::ImDrawList* self,
												 ...
												 ImU32 col, int num_segments)
{
	reinterpret_cast<::ImDrawList*>(self)->AddNgonFilled(ConvertToCPP_ImVec2(center),
														 radius, col, num_segments);
}
...
{
	reinterpret_cast<::ImDrawList*>(self)->AddEllipse(ConvertToCPP_ImVec2(center),
													  ConvertToCPP_ImVec2(radius), col);
}

CIMGUI_API void cimgui::ImDrawList_AddEllipseEx(cimgui::ImDrawList* self,
												cimgui::ImVec2 center,
												cimgui::ImVec2 radius, ImU32 col,
...
{
	reinterpret_cast<::ImDrawList*>(self)->AddEllipseFilled(
		ConvertToCPP_ImVec2(center), ConvertToCPP_ImVec2(radius), col, rot, num_segments);
}

CIMGUI_API void cimgui::ImDrawList_AddText(cimgui::ImDrawList* self, cimgui::ImVec2 pos,
...
CIMGUI_API void cimgui::ImDrawList_AddTextEx(cimgui::ImDrawList* self, cimgui::ImVec2 pos,
											 ImU32 col, const char* text_begin,
											 const char* text_end)
{
	reinterpret_cast<::ImDrawList*>(self)->AddText(ConvertToCPP_ImVec2(pos), col,
...
CIMGUI_API void cimgui::ImDrawList_AddTextImFontPtr(cimgui::ImDrawList* self,
													cimgui::ImFont* font, float font_size,
													cimgui::ImVec2 pos, ImU32 col,
													const char* text_begin)
{
	reinterpret_cast<::ImDrawList*>(self)->AddText(reinterpret_cast<::ImFont*>(font),
												   font_size, ConvertToCPP_ImVec2(pos),
												   col, text_begin);
}

CIMGUI_API void cimgui::ImDrawList_AddTextImFontPtrEx(
	cimgui::ImDrawList* self, cimgui::ImFont* font, float font_size, cimgui::ImVec2 pos,
	ImU32 col, const char* text_begin, const char* text_end, float wrap_width,
	const cimgui::ImVec4* cpu_fine_clip_rect)
{
	reinterpret_cast<::ImDrawList*>(self)->AddText(
		reinterpret_cast<::ImFont*>(font), font_size, ConvertToCPP_ImVec2(pos), col,
		text_begin, text_end, wrap_width,
		reinterpret_cast<const ::ImVec4*>(cpu_fine_clip_rect));
}

CIMGUI_API void cimgui::ImDrawList_AddBezierCubic(cimgui::ImDrawList* self,
												  cimgui::ImVec2 p1, cimgui::ImVec2 p2,
												  cimgui::ImVec2 p3, cimgui::ImVec2 p4,
...
CIMGUI_API void cimgui::ImDrawList_AddBezierQuadratic(cimgui::ImDrawList* self,
													  cimgui::ImVec2 p1,
													  cimgui::ImVec2 p2,
													  cimgui::ImVec2 p3, ImU32 col,
											   ...
											   ImDrawFlags flags, float thickness)
{
	reinterpret_cast<::ImDrawList*>(self)->AddPolyline(
		reinterpret_cast<const ::ImVec2*>(points), num_points, col, flags, thickness);
}

CIMGUI_API void cimgui::ImDrawList_AddConvexPolyFilled(cimgui::ImDrawList* self,
													   const cimgui::ImVec2* points,
													   int num_points, ImU32 col)
{
...
CIMGUI_API void cimgui::ImDrawList_AddConcavePolyFilled(cimgui::ImDrawList* self,
														const cimgui::ImVec2* points,
														int num_points, ImU32 col)
{
...
CIMGUI_API void cimgui::ImDrawList_AddImage(cimgui::ImDrawList* self,
											...
											cimgui::ImVec2 p_min, cimgui::ImVec2 p_max)
{
	reinterpret_cast<::ImDrawList*>(self)->AddImage(ConvertToCPP_ImTextureRef(tex_ref),
													ConvertToCPP_ImVec2(p_min),
													ConvertToCPP_ImVec2(p_max));
}

CIMGUI_API void cimgui::ImDrawList_AddImageEx(cimgui::ImDrawList* self,
											  cimgui::ImTextureRef tex_ref,
											  cimgui::ImVec2 p_min, cimgui::ImVec2 p_max,
											  cimgui::ImVec2 uv_min,
											  cimgui::ImVec2 uv_max, ImU32 col)
{
	reinterpret_cast<::ImDrawList*>(self)->AddImage(
		ConvertToCPP_ImTextureRef(tex_ref), ConvertToCPP_ImVec2(p_min),
		ConvertToCPP_ImVec2(p_max), ConvertToCPP_ImVec2(uv_min),
		ConvertToCPP_ImVec2(uv_max), col);
}

CIMGUI_API void cimgui::ImDrawList_AddImageQuad(cimgui::ImDrawList* self,
												cimgui::ImTextureRef tex_ref,
												cimgui::ImVec2 p1, cimgui::ImVec2 p2,
												cimgui::ImVec2 p3, cimgui::ImVec2 p4)
{
	reinterpret_cast<::ImDrawList*>(self)->AddImageQuad(
		ConvertToCPP_ImTextureRef(tex_ref), ConvertToCPP_ImVec2(p1),
		ConvertToCPP_ImVec2(p2), ConvertToCPP_ImVec2(p3), ConvertToCPP_ImVec2(p4));
}

CIMGUI_API void cimgui::ImDrawList_AddImageQuadEx(
	cimgui::ImDrawList* self, cimgui::ImTextureRef tex_ref, cimgui::ImVec2 p1,
	...
	cimgui::ImVec2 uv2, cimgui::ImVec2 uv3, cimgui::ImVec2 uv4, ImU32 col)
{
	reinterpret_cast<::ImDrawList*>(self)->AddImageQuad(
		ConvertToCPP_ImTextureRef(tex_ref), ConvertToCPP_ImVec2(p1),
		ConvertToCPP_ImVec2(p2), ConvertToCPP_ImVec2(p3), ConvertToCPP_ImVec2(p4),
		ConvertToCPP_ImVec2(uv1), ConvertToCPP_ImVec2(uv2), ConvertToCPP_ImVec2(uv3),
		ConvertToCPP_ImVec2(uv4), col);
}

CIMGUI_API void cimgui::ImDrawList_AddImageRounded(
	cimgui::ImDrawList* self, cimgui::ImTextureRef tex_ref, cimgui::ImVec2 p_min,
	...
	float rounding, ImDrawFlags flags)
{
	reinterpret_cast<::ImDrawList*>(self)->AddImageRounded(
		ConvertToCPP_ImTextureRef(tex_ref), ConvertToCPP_ImVec2(p_min),
		ConvertToCPP_ImVec2(p_max), ConvertToCPP_ImVec2(uv_min),
		ConvertToCPP_ImVec2(uv_max), col, rounding, flags);
}
...
CIMGUI_API void cimgui::ImDrawList_PathLineToMergeDuplicate(cimgui::ImDrawList* self,
															cimgui::ImVec2 pos)
{
...
CIMGUI_API void cimgui::ImDrawList_PathStroke(cimgui::ImDrawList* self, ImU32 col,
											  ImDrawFlags flags, float thickness)
{
...
{
	reinterpret_cast<::ImDrawList*>(self)->PathArcTo(ConvertToCPP_ImVec2(center), radius,
...
CIMGUI_API void cimgui::ImDrawList_PathEllipticalArcTo(cimgui::ImDrawList* self,
													   cimgui::ImVec2 center,
													   cimgui::ImVec2 radius, float rot,
													   float a_min, float a_max)
{
	reinterpret_cast<::ImDrawList*>(self)->PathEllipticalArcTo(
		ConvertToCPP_ImVec2(center), ConvertToCPP_ImVec2(radius), rot, a_min, a_max);
}

CIMGUI_API void cimgui::ImDrawList_PathEllipticalArcToEx(cimgui::ImDrawList* self,
														 cimgui::ImVec2 center,
														 cimgui::ImVec2 radius, float rot,
														 float a_min, float a_max,
														 int num_segments)
...
CIMGUI_API void cimgui::ImDrawList_PathBezierCubicCurveTo(cimgui::ImDrawList* self,
														  cimgui::ImVec2 p2,
														  cimgui::ImVec2 p3,
														  cimgui::ImVec2 p4,
														  int num_segments)
...
CIMGUI_API void cimgui::ImDrawList_PathBezierQuadraticCurveTo(cimgui::ImDrawList* self,
															  cimgui::ImVec2 p2,
															  cimgui::ImVec2 p3,
															  int num_segments)
{
...
CIMGUI_API void cimgui::ImDrawList_PathRect(cimgui::ImDrawList* self,
											...
											cimgui::ImVec2 rect_max, float rounding,
											ImDrawFlags flags)
{
	reinterpret_cast<::ImDrawList*>(self)->PathRect(
		ConvertToCPP_ImVec2(rect_min), ConvertToCPP_ImVec2(rect_max), rounding, flags);
}
...
CIMGUI_API void cimgui::ImDrawList_AddCallbackEx(cimgui::ImDrawList* self,
												 cimgui::ImDrawCallback callback,
...
CIMGUI_API cimgui::ImDrawList* cimgui::ImDrawList_CloneOutput(
	const cimgui::ImDrawList* self)
{
	return reinterpret_cast<::cimgui::ImDrawList*>(
...
CIMGUI_API void cimgui::ImDrawList_PrimReserve(cimgui::ImDrawList* self, int idx_count,
											   int vtx_count)
{
	reinterpret_cast<::ImDrawList*>(self)->PrimReserve(idx_count, vtx_count);
}

CIMGUI_API void cimgui::ImDrawList_PrimUnreserve(cimgui::ImDrawList* self, int idx_count,
												 int vtx_count)
{
	reinterpret_cast<::ImDrawList*>(self)->PrimUnreserve(idx_count, vtx_count);
}

CIMGUI_API void cimgui::ImDrawList_PrimRect(cimgui::ImDrawList* self, cimgui::ImVec2 a,
											cimgui::ImVec2 b, ImU32 col)
{
	reinterpret_cast<::ImDrawList*>(self)->PrimRect(ConvertToCPP_ImVec2(a),
													ConvertToCPP_ImVec2(b), col);
}

CIMGUI_API void cimgui::ImDrawList_PrimRectUV(cimgui::ImDrawList* self, cimgui::ImVec2 a,
											  cimgui::ImVec2 b, cimgui::ImVec2 uv_a,
...
CIMGUI_API void cimgui::ImDrawList_PrimQuadUV(cimgui::ImDrawList* self, cimgui::ImVec2 a,
											  cimgui::ImVec2 b, cimgui::ImVec2 c,
											  cimgui::ImVec2 d, cimgui::ImVec2 uv_a,
											  cimgui::ImVec2 uv_b, cimgui::ImVec2 uv_c,
											  cimgui::ImVec2 uv_d, ImU32 col)
{
	reinterpret_cast<::ImDrawList*>(self)->PrimQuadUV(
		ConvertToCPP_ImVec2(a), ConvertToCPP_ImVec2(b), ConvertToCPP_ImVec2(c),
		ConvertToCPP_ImVec2(d), ConvertToCPP_ImVec2(uv_a), ConvertToCPP_ImVec2(uv_b),
		ConvertToCPP_ImVec2(uv_c), ConvertToCPP_ImVec2(uv_d), col);
}

CIMGUI_API void cimgui::ImDrawList_PrimWriteVtx(cimgui::ImDrawList* self,
												cimgui::ImVec2 pos, cimgui::ImVec2 uv,
												ImU32 col)
{
	reinterpret_cast<::ImDrawList*>(self)->PrimWriteVtx(ConvertToCPP_ImVec2(pos),
...
CIMGUI_API void cimgui::ImDrawList_PrimVtx(cimgui::ImDrawList* self, cimgui::ImVec2 pos,
...
CIMGUI_API void cimgui::ImDrawList_PushTextureID(cimgui::ImDrawList* self,
												 cimgui::ImTextureRef tex_ref)
{
	reinterpret_cast<::ImDrawList*>(self)->PushTextureID(
		ConvertToCPP_ImTextureRef(tex_ref));
}
...
CIMGUI_API void cimgui::ImDrawList__SetDrawListSharedData(
	cimgui::ImDrawList* self, cimgui::ImDrawListSharedData* data)
{
	reinterpret_cast<::ImDrawList*>(self)->_SetDrawListSharedData(
...
CIMGUI_API void cimgui::ImDrawList__SetTexture(cimgui::ImDrawList* self,
											   cimgui::ImTextureRef tex_ref)
{
	reinterpret_cast<::ImDrawList*>(self)->_SetTexture(
		ConvertToCPP_ImTextureRef(tex_ref));
}

CIMGUI_API int cimgui::ImDrawList__CalcCircleAutoSegmentCount(
	const cimgui::ImDrawList* self, float radius)
{
	return reinterpret_cast<const ::ImDrawList*>(self)->_CalcCircleAutoSegmentCount(
		radius);
}

CIMGUI_API void cimgui::ImDrawList__PathArcToFastEx(cimgui::ImDrawList* self,
													cimgui::ImVec2 center, float radius,
...
CIMGUI_API void cimgui::ImTextureData_Create(cimgui::ImTextureData* self,
											 cimgui::ImTextureFormat format, int w, int h)
{
	reinterpret_cast<::ImTextureData*>(self)->Create(
		static_cast<::ImTextureFormat>(format), w, h);
}
...
CIMGUI_API void* cimgui::ImTextureData_GetPixelsAt(cimgui::ImTextureData* self, int x,
												   int y)
{
...
CIMGUI_API cimgui::ImTextureRef cimgui::ImTextureData_GetTexRef(
	cimgui::ImTextureData* self)
{
	return ConvertFromCPP_ImTextureRef(
		reinterpret_cast<::ImTextureData*>(self)->GetTexRef());
}
...
CIMGUI_API void cimgui::ImTextureData_SetTexID(cimgui::ImTextureData* self,
											   ImTextureID tex_id)
{
	reinterpret_cast<::ImTextureData*>(self)->SetTexID(tex_id);
}

CIMGUI_API void cimgui::ImTextureData_SetStatus(cimgui::ImTextureData* self,
												cimgui::ImTextureStatus status)
{
	reinterpret_cast<::ImTextureData*>(self)->SetStatus(
		static_cast<::ImTextureStatus>(status));
}

CIMGUI_API void cimgui::ImFontGlyphRangesBuilder_Clear(
...
CIMGUI_API void cimgui::ImFontGlyphRangesBuilder_SetBit(
	cimgui::ImFontGlyphRangesBuilder* self, size_t n)
{
	reinterpret_cast<::ImFontGlyphRangesBuilder*>(self)->SetBit(n);
}

CIMGUI_API void cimgui::ImFontGlyphRangesBuilder_AddChar(
	cimgui::ImFontGlyphRangesBuilder* self, ImWchar c)
{
	reinterpret_cast<::ImFontGlyphRangesBuilder*>(self)->AddChar(c);
}

CIMGUI_API void cimgui::ImFontGlyphRangesBuilder_AddText(
	cimgui::ImFontGlyphRangesBuilder* self, const char* text, const char* text_end)
{
...
CIMGUI_API cimgui::ImFont* cimgui::ImFontAtlas_AddFont(
	cimgui::ImFontAtlas* self, const cimgui::ImFontConfig* font_cfg)
{
...
CIMGUI_API cimgui::ImFont* cimgui::ImFontAtlas_AddFontDefault(
	cimgui::ImFontAtlas* self, const cimgui::ImFontConfig* font_cfg)
{
	return reinterpret_cast<::cimgui::ImFont*>(
...
CIMGUI_API cimgui::ImFont* cimgui::ImFontAtlas_AddFontDefaultVector(
	cimgui::ImFontAtlas* self, const cimgui::ImFontConfig* font_cfg)
{
	return reinterpret_cast<::cimgui::ImFont*>(
		reinterpret_cast<::ImFontAtlas*>(self)->AddFontDefaultVector(
			reinterpret_cast<const ::ImFontConfig*>(font_cfg)));
}

CIMGUI_API cimgui::ImFont* cimgui::ImFontAtlas_AddFontDefaultBitmap(
	cimgui::ImFontAtlas* self, const cimgui::ImFontConfig* font_cfg)
{
	return reinterpret_cast<::cimgui::ImFont*>(
		reinterpret_cast<::ImFontAtlas*>(self)->AddFontDefaultBitmap(
			reinterpret_cast<const ::ImFontConfig*>(font_cfg)));
}
	...
	const cimgui::ImFontConfig* font_cfg, const ImWchar* glyph_ranges)
{
	return reinterpret_cast<::cimgui::ImFont*>(
		reinterpret_cast<::ImFontAtlas*>(self)->AddFontFromFileTTF(
			filename, size_pixels, reinterpret_cast<const ::ImFontConfig*>(font_cfg),
			glyph_ranges));
}

CIMGUI_API cimgui::ImFont* cimgui::ImFontAtlas_AddFontFromMemoryTTF(
	cimgui::ImFontAtlas* self, void* font_data, int font_data_size, float size_pixels,
	const cimgui::ImFontConfig* font_cfg, const ImWchar* glyph_ranges)
{
	return reinterpret_cast<::cimgui::ImFont*>(
...
CIMGUI_API cimgui::ImFont* cimgui::ImFontAtlas_AddFontFromMemoryCompressedBase85TTF(
	cimgui::ImFontAtlas* self, const char* compressed_font_data_base85, float size_pixels,
	const cimgui::ImFontConfig* font_cfg, const ImWchar* glyph_ranges)
{
	return reinterpret_cast<::cimgui::ImFont*>(
		reinterpret_cast<::ImFontAtlas*>(self)->AddFontFromMemoryCompressedBase85TTF(
			compressed_font_data_base85, size_pixels,
			reinterpret_cast<const ::ImFontConfig*>(font_cfg), glyph_ranges));
}

CIMGUI_API void cimgui::ImFontAtlas_RemoveFont(cimgui::ImFontAtlas* self,
											   cimgui::ImFont* font)
{
...
CIMGUI_API void cimgui::ImFontAtlas_SetFontLoader(cimgui::ImFontAtlas* self,
												  const cimgui::ImFontLoader* font_loader)
{
	reinterpret_cast<::ImFontAtlas*>(self)->SetFontLoader(
...
CIMGUI_API void cimgui::ImFontAtlas_GetTexDataAsAlpha8(cimgui::ImFontAtlas* self,
...
CIMGUI_API void cimgui::ImFontAtlas_GetTexDataAsRGBA32(cimgui::ImFontAtlas* self,
													   unsigned char** out_pixels,
													   int* out_width, int* out_height,
													   int* out_bytes_per_pixel)
{
...
CIMGUI_API void cimgui::ImFontAtlas_SetTexIDImTextureRef(cimgui::ImFontAtlas* self,
														 cimgui::ImTextureRef id)
{
...
CIMGUI_API const ImWchar* cimgui::ImFontAtlas_GetGlyphRangesDefault(
...
CIMGUI_API const ImWchar* cimgui::ImFontAtlas_GetGlyphRangesGreek(
	cimgui::ImFontAtlas* self)
{
	return reinterpret_cast<::ImFontAtlas*>(self)->GetGlyphRangesGreek();
}

CIMGUI_API const ImWchar* cimgui::ImFontAtlas_GetGlyphRangesKorean(
	cimgui::ImFontAtlas* self)
{
...
CIMGUI_API const ImWchar* cimgui::ImFontAtlas_GetGlyphRangesChineseFull(
	cimgui::ImFontAtlas* self)
{
	return reinterpret_cast<::ImFontAtlas*>(self)->GetGlyphRangesChineseFull();
}

CIMGUI_API const ImWchar* cimgui::ImFontAtlas_GetGlyphRangesChineseSimplifiedCommon(
	cimgui::ImFontAtlas* self)
{
...
CIMGUI_API const ImWchar* cimgui::ImFontAtlas_GetGlyphRangesCyrillic(
	cimgui::ImFontAtlas* self)
{
	return reinterpret_cast<::ImFontAtlas*>(self)->GetGlyphRangesCyrillic();
}

CIMGUI_API const ImWchar* cimgui::ImFontAtlas_GetGlyphRangesThai(
	cimgui::ImFontAtlas* self)
{
	return reinterpret_cast<::ImFontAtlas*>(self)->GetGlyphRangesThai();
}

CIMGUI_API const ImWchar* cimgui::ImFontAtlas_GetGlyphRangesVietnamese(
	cimgui::ImFontAtlas* self)
{
...
CIMGUI_API ImFontAtlasRectId cimgui::ImFontAtlas_AddCustomRect(
	cimgui::ImFontAtlas* self, int width, int height, cimgui::ImFontAtlasRect* out_r)
{
	return reinterpret_cast<::ImFontAtlas*>(self)->AddCustomRect(
		width, height, reinterpret_cast<::ImFontAtlasRect*>(out_r));
}

CIMGUI_API void cimgui::ImFontAtlas_RemoveCustomRect(cimgui::ImFontAtlas* self,
													 ImFontAtlasRectId id)
{
	reinterpret_cast<::ImFontAtlas*>(self)->RemoveCustomRect(id);
}

CIMGUI_API bool cimgui::ImFontAtlas_GetCustomRect(const cimgui::ImFontAtlas* self,
												  ImFontAtlasRectId id,
												  cimgui::ImFontAtlasRect* out_r)
{
	return reinterpret_cast<const ::ImFontAtlas*>(self)->GetCustomRect(
		id, reinterpret_cast<::ImFontAtlasRect*>(out_r));
}

#	ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS

CIMGUI_API ImFontAtlasRectId
cimgui::ImFontAtlas_AddCustomRectRegular(cimgui::ImFontAtlas* self, int w, int h)
{
	return reinterpret_cast<::ImFontAtlas*>(self)->AddCustomRectRegular(w, h);
}

CIMGUI_API const cimgui::ImFontAtlasRect* cimgui::ImFontAtlas_GetCustomRectByIndex(
	cimgui::ImFontAtlas* self, ImFontAtlasRectId id)
{
	return reinterpret_cast<const ::cimgui::ImFontAtlasRect*>(
		reinterpret_cast<::ImFontAtlas*>(self)->GetCustomRectByIndex(id));
}

CIMGUI_API void cimgui::ImFontAtlas_CalcCustomRectUV(const cimgui::ImFontAtlas* self,
													 const cimgui::ImFontAtlasRect* r,
													 cimgui::ImVec2* out_uv_min,
													 cimgui::ImVec2* out_uv_max)
{
	reinterpret_cast<const ::ImFontAtlas*>(self)->CalcCustomRectUV(
		reinterpret_cast<const ::ImFontAtlasRect*>(r),
		reinterpret_cast<::ImVec2*>(out_uv_min), reinterpret_cast<::ImVec2*>(out_uv_max));
}

CIMGUI_API ImFontAtlasRectId cimgui::ImFontAtlas_AddCustomRectFontGlyph(
	cimgui::ImFontAtlas* self, cimgui::ImFont* font, ImWchar codepoint, int w, int h,
	float advance_x, cimgui::ImVec2 offset)
{
	return reinterpret_cast<::ImFontAtlas*>(self)->AddCustomRectFontGlyph(
		reinterpret_cast<::ImFont*>(font), codepoint, w, h, advance_x,
		ConvertToCPP_ImVec2(offset));
}

CIMGUI_API ImFontAtlasRectId cimgui::ImFontAtlas_AddCustomRectFontGlyphForSize(
	cimgui::ImFontAtlas* self, cimgui::ImFont* font, float font_size, ImWchar codepoint,
	int w, int h, float advance_x, cimgui::ImVec2 offset)
{
	return reinterpret_cast<::ImFontAtlas*>(self)->AddCustomRectFontGlyphForSize(
		reinterpret_cast<::ImFont*>(font), font_size, codepoint, w, h, advance_x,
		ConvertToCPP_ImVec2(offset));
}
...
CIMGUI_API cimgui::ImFontGlyph* cimgui::ImFontBaked_FindGlyph(cimgui::ImFontBaked* self,
															  ImWchar c)
{
	return reinterpret_cast<::cimgui::ImFontGlyph*>(
		reinterpret_cast<::ImFontBaked*>(self)->FindGlyph(c));
}

CIMGUI_API cimgui::ImFontGlyph* cimgui::ImFontBaked_FindGlyphNoFallback(
	cimgui::ImFontBaked* self, ImWchar c)
{
	return reinterpret_cast<::cimgui::ImFontGlyph*>(
		reinterpret_cast<::ImFontBaked*>(self)->FindGlyphNoFallback(c));
}
...
CIMGUI_API cimgui::ImFontBaked* cimgui::ImFont_GetFontBaked(cimgui::ImFont* self,
															float font_size)
{
	return reinterpret_cast<::cimgui::ImFontBaked*>(
		reinterpret_cast<::ImFont*>(self)->GetFontBaked(font_size));
}

CIMGUI_API cimgui::ImFontBaked* cimgui::ImFont_GetFontBakedEx(cimgui::ImFont* self,
															  float font_size,
															  float density)
{
	return reinterpret_cast<::cimgui::ImFontBaked*>(
		reinterpret_cast<::ImFont*>(self)->GetFontBaked(font_size, density));
}

CIMGUI_API cimgui::ImVec2 cimgui::ImFont_CalcTextSizeA(cimgui::ImFont* self, float size,
													   float max_width, float wrap_width,
													   const char* text_begin)
{
	return ConvertFromCPP_ImVec2(reinterpret_cast<::ImFont*>(self)->CalcTextSizeA(
...
CIMGUI_API cimgui::ImVec2 cimgui::ImFont_CalcTextSizeAEx(
	...
	const char* text_begin, const char* text_end, const char** out_remaining)
{
	return ConvertFromCPP_ImVec2(reinterpret_cast<::ImFont*>(self)->CalcTextSizeA(
		size, max_width, wrap_width, text_begin, text_end, out_remaining));
}

CIMGUI_API const char* cimgui::ImFont_CalcWordWrapPosition(cimgui::ImFont* self,
														   float size, const char* text,
														   const char* text_end,
														   float wrap_width)
{
	return reinterpret_cast<::ImFont*>(self)->CalcWordWrapPosition(size, text, text_end,
																   wrap_width);
}

CIMGUI_API void cimgui::ImFont_RenderChar(cimgui::ImFont* self,
										  cimgui::ImDrawList* draw_list, float size,
										  cimgui::ImVec2 pos, ImU32 col, ImWchar c)
{
	reinterpret_cast<::ImFont*>(self)->RenderChar(
		reinterpret_cast<::ImDrawList*>(draw_list), size, ConvertToCPP_ImVec2(pos), col,
		c);
}

CIMGUI_API void cimgui::ImFont_RenderCharEx(cimgui::ImFont* self,
											cimgui::ImDrawList* draw_list, float size,
											cimgui::ImVec2 pos, ImU32 col, ImWchar c,
											const cimgui::ImVec4* cpu_fine_clip)
{
	reinterpret_cast<::ImFont*>(self)->RenderChar(
		reinterpret_cast<::ImDrawList*>(draw_list), size, ConvertToCPP_ImVec2(pos), col,
		c, reinterpret_cast<const ::ImVec4*>(cpu_fine_clip));
}

CIMGUI_API void cimgui::ImFont_RenderText(cimgui::ImFont* self,
										  cimgui::ImDrawList* draw_list, float size,
										  cimgui::ImVec2 pos, ImU32 col,
										  cimgui::ImVec4 clip_rect,
										  const char* text_begin, const char* text_end,
										  float wrap_width, ImDrawTextFlags flags)
{
	reinterpret_cast<::ImFont*>(self)->RenderText(
		reinterpret_cast<::ImDrawList*>(draw_list), size, ConvertToCPP_ImVec2(pos), col,
		ConvertToCPP_ImVec4(clip_rect), text_begin, text_end, wrap_width, flags);
}

#	ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS

CIMGUI_API const char* cimgui::ImFont_CalcWordWrapPositionA(cimgui::ImFont* self,
															float scale, const char* text,
															const char* text_end,
															float wrap_width)
{
	return reinterpret_cast<::ImFont*>(self)->CalcWordWrapPositionA(scale, text, text_end,
																	wrap_width);
}
...
CIMGUI_API void cimgui::ImFont_AddRemapChar(cimgui::ImFont* self, ImWchar from_codepoint,
											ImWchar to_codepoint)
{
...
CIMGUI_API cimgui::ImVec2 cimgui::ImGuiViewport_GetCenter(
	const cimgui::ImGuiViewport* self)
{
	return ConvertFromCPP_ImVec2(
		reinterpret_cast<const ::ImGuiViewport*>(self)->GetCenter());
}

CIMGUI_API cimgui::ImVec2 cimgui::ImGuiViewport_GetWorkCenter(
	const cimgui::ImGuiViewport* self)
{
	return ConvertFromCPP_ImVec2(
...
CIMGUI_API void cimgui::ImGuiPlatformIO_ClearPlatformHandlers(
	cimgui::ImGuiPlatformIO* self)
{
	reinterpret_cast<::ImGuiPlatformIO*>(self)->ClearPlatformHandlers();
}

CIMGUI_API void cimgui::ImGuiPlatformIO_ClearRendererHandlers(
	cimgui::ImGuiPlatformIO* self)
{
...
CIMGUI_API void cimgui::ImGui_ImageImVec4(cimgui::ImTextureRef tex_ref,
										  cimgui::ImVec2 image_size, cimgui::ImVec2 uv0,
										  cimgui::ImVec2 uv1, cimgui::ImVec4 tint_col,
										  cimgui::ImVec4 border_col)
{
	::ImGui::Image(ConvertToCPP_ImTextureRef(tex_ref), ConvertToCPP_ImVec2(image_size),
				   ConvertToCPP_ImVec2(uv0), ConvertToCPP_ImVec2(uv1),
				   ConvertToCPP_ImVec4(tint_col), ConvertToCPP_ImVec4(border_col));
}
...
CIMGUI_API bool cimgui::ImGui_ComboObsolete(const char* label, int* current_item,
...
CIMGUI_API bool cimgui::ImGui_ListBoxObsolete(const char* label, int* current_item,
